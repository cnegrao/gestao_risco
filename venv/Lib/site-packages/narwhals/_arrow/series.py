...
class ArrowSeries(EagerSeries["ChunkedArrayAny"]):
    ...
    def _with_native(self, series: ArrayOrScalar, *, preserve_broadcast: bool = False) -> Self:
    ...
    def from_iterable(
        ...
        *,
        ...
        return cls.from_native(chunked_array([data], dtype_pa), name=name, context=context)
    ...
    def from_numpy(cls, data: Into1DArray, /, *, context: _FullContext) -> Self:
        return cls.from_iterable(data if is_numpy_array_1d(data) else [data], context=context)
    ...
    def median(self, *, _return_py_scalar: bool = True) -> float:
        ...
        return maybe_extract_py_scalar(pc.approximate_median(self.native), _return_py_scalar)
    ...
    def sum(self, *, _return_py_scalar: bool = True) -> float:
        return maybe_extract_py_scalar(pc.sum(self.native, min_count=0), _return_py_scalar)
    ...
    def std(self, ddof: int, *, _return_py_scalar: bool = True) -> float:
        return maybe_extract_py_scalar(pc.stddev(self.native, ddof=ddof), _return_py_scalar)

    def var(self, ddof: int, *, _return_py_scalar: bool = True) -> float:
        return maybe_extract_py_scalar(pc.variance(self.native, ddof=ddof), _return_py_scalar)
    ...
    def any(self, *, _return_py_scalar: bool = True) -> bool:
        return maybe_extract_py_scalar(pc.any(self.native, min_count=0), _return_py_scalar)

    def all(self, *, _return_py_scalar: bool = True) -> bool:
        return maybe_extract_py_scalar(pc.all(self.native, min_count=0), _return_py_scalar)

    def is_between(self, lower_bound: Any, upper_bound: Any, closed: ClosedInterval) -> Self:
    ...
    def fill_null(
        ...
        if value is not None:
            ...
            fill_func = pc.fill_null_forward if strategy == "forward" else pc.fill_null_backward
    ...
    def to_dummies(self, *, separator: str, drop_first: bool) -> ArrowDataFrame:
        ...
        cols = [
            {null_col_pa: null_col_pl}.get(f"{name}{separator}{v}", f"{name}{separator}{v}")
    ...
    def mode(self) -> ArrowSeries:
        ...
        col_token = generate_temporary_column_name(n_bytes=8, columns=[self.name])
        counts = self.value_counts(name=col_token, normalize=False, sort=False, parallel=False)
        return counts.filter(
            plx.col(col_token) == plx.col(col_token).max().broadcast(kind=ExprKind.AGGREGATION)
    ...
    def rolling_sum(self, window_size: int, *, min_samples: int, center: bool) -> Self:
        ...
        rolling_sum = (
            cum_sum - cum_sum.shift(window_size).fill_null(value=0, strategy=None, limit=None)
            if window_size != 0
            else cum_sum
        )
    ...
    def rolling_var(self, window_size: int, *, min_samples: int, center: bool, ddof: int) -> Self:
        ...
        rolling_sum_sq = (
            cum_sum_sq - cum_sum_sq.shift(window_size).fill_null(value=0, strategy=None, limit=None)
            if window_size != 0
            else cum_sum_sq
        )
    ...
    def hist(  # noqa: PLR0915
        ...
        *,
        ...
        data_count = pc.sum(
            pc.invert(pc.or_(pc.is_nan(self.native), pc.is_null(self.native))).cast(pa.uint8()),
            min_count=0,
        )
        if bins is not None:
            if len(bins) < 2:
    ...
    def __contains__(self, other: Any) -> bool:
        ...
        try:
            ...
            return maybe_extract_py_scalar(pc.is_in(other_, self.native), return_py_scalar=True)
