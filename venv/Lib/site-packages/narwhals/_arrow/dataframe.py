...
class ArrowDataFrame(EagerDataFrame["ArrowSeries", "ArrowExpr", "pa.Table", "ChunkedArrayAny"]):
    def __init__(
        self,
    ...
    def _select_multi_index(self, columns: SizedMultiIndexSelector[ChunkedArrayAny]) -> Self:
    ...
    def _select_multi_name(self, columns: SizedMultiNameSelector[ChunkedArrayAny]) -> Self:
    ...
    def to_dict(self, *, as_series: bool) -> dict[str, ArrowSeries] | dict[str, list[Any]]:
    ...
    def with_row_index(self, name: str) -> Self:
        ...
        return self._with_native(df.append_column(name, row_indices).select([name, *cols]))

    def filter(self: ArrowDataFrame, predicate: ArrowExpr | list[bool | None]) -> ArrowDataFrame:
        if isinstance(predicate, list):
            mask_native: Mask | ChunkedArrayAny = predicate
        else:
            # `[0]` is safe as the predicate's expression only returns a single column
            mask_native = self._evaluate_into_exprs(predicate)[0].native
        return self._with_native(self.native.filter(mask_native), validate_column_names=False)
    ...
    def head(self, n: int) -> Self:
        ...
        if n >= 0:
            ...
            num_rows = df.num_rows
            return self._with_native(df.slice(0, max(0, num_rows + n)), validate_column_names=False)

    def tail(self, n: int) -> Self:
        df = self.native
        if n >= 0:
            num_rows = df.num_rows
            return self._with_native(df.slice(max(0, num_rows - n)), validate_column_names=False)
        else:
    ...
    def unique(
        ...
        *,
        ...
        if keep in {"any", "first", "last"}:
            ...
            return self._with_native(self.native.take(keep_idx_native), validate_column_names=False)
