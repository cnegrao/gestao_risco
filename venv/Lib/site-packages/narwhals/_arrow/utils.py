...
def native_to_narwhals_dtype(dtype: pa.DataType, version: Version) -> DType:
    ...
    if is_fixed_size_list(dtype):
        return dtypes.Array(native_to_narwhals_dtype(dtype.value_type, version), dtype.list_size)
...
def narwhals_to_native_dtype(dtype: DType | type[DType], version: Version) -> pa.DataType:
    ...
    if isinstance_or_issubclass(dtype, dtypes.Datetime):
        return pa.timestamp(
            dtype.time_unit, tz=dtype.time_zone
...
YMD_RE_NO_SEP = (
    r"^(?P<year>(?:[12][0-9])?[0-9]{2})(?P<month>0[1-9]|1[0-2])(?P<day>0[1-9]|[12][0-9]|3[01])$"
)
...
def pad_series(series: ArrowSeries, *, window_size: int, center: bool) -> tuple[ArrowSeries, int]:
