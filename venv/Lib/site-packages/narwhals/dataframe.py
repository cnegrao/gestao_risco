...
class BaseFrame(Generic[_FrameT]):
    ...
    def select(
        ...
        **named_exprs: IntoExpr,
        ...
        if flat_exprs and all(isinstance(x, str) for x in flat_exprs) and not named_exprs:
            # fast path!
            try:
                return self._with_compliant(self._compliant_frame.simple_select(*flat_exprs))
    ...
    def filter(
        ...
        **constraints: Any,
        ...
        if len(predicates) == 1 and is_list_of(predicates[0], bool):
            ...
            predicate = plx.all_horizontal(*chain(compliant_predicates, compliant_constraints))
    ...
    def join(
        ...
        *,
        ...
        if how not in (_supported_joins := ("inner", "left", "full", "cross", "anti", "semi")):
            msg = f"Only the following join strategies are supported: {_supported_joins}; found '{how}'."
            raise NotImplementedError(msg)

        if how == "cross" and (left_on is not None or right_on is not None or on is not None):
        ...
        if how != "cross" and (on is not None and (left_on is not None or right_on is not None)):
    ...
    def gather_every(self, n: int, offset: int = 0) -> Self:
        return self._with_compliant(self._compliant_frame.gather_every(n=n, offset=offset))
    ...
    def join_asof(
        ...
        *,
        ...
        if (on is not None) and (left_on is not None or right_on is not None):
            ...
            raise ValueError(msg)
        if (by is None) and (
            (by_left is None and by_right is not None) or (by_left is not None and by_right is None)
        ):
    ...
    def explode(self, columns: str | Sequence[str], *more_columns: str) -> Self:
        to_explode = (
            [columns, *more_columns] if isinstance(columns, str) else [*columns, *more_columns]
        )
...
class DataFrame(BaseFrame[DataFrameT]):
    ...
    def lazy(
        ...
        if lazy_backend is not None and lazy_backend not in supported_lazy_backends:
            ...
            raise ValueError(msg)
        return self._lazyframe(self._compliant_frame.lazy(backend=lazy_backend), level="lazy")
    ...
    @overload
    def to_dict(self, *, as_series: bool) -> dict[str, Series[Any]] | dict[str, list[Any]]: ...
    def to_dict(self, *, as_series: bool = True) -> dict[str, Series[Any]] | dict[str, list[Any]]:
        ...
        if as_series:
            return {
                ...
                for key, value in self._compliant_frame.to_dict(as_series=as_series).items()
            }
    ...
    def group_by(
        ...
        _keys = [k if is_expr else col(k) for k, is_expr in zip(flat_keys, key_is_expr_or_series)]
...
class LazyFrame(BaseFrame[FrameT]):
    ...
    def unique(
        ...
        *,
        ...
        if isinstance(subset, str):
            subset = [subset]
        return self._with_compliant(self._compliant_frame.unique(subset=subset, keep=keep))
