...
def evaluate_output_names_and_aliases(
    ...
    aliases = (
        output_names if expr._alias_output_names is None else expr._alias_output_names(output_names)
    )
    if exclude:
...
def check_expressions_preserve_length(*args: IntoExpr, function_name: str) -> None:
    ...
    if not all(
        (is_expr(x) and x._metadata.kind.preserves_length()) or isinstance(x, (str, Series))
        for x in args
    ):
...
def apply_n_ary_operation(
    ...
    *comparands: IntoExpr | NonNestedLiteral | _1DArray,
    ...
    compliant_exprs = (
        extract_compliant(plx, comparand, str_as_lit=str_as_lit) for comparand in comparands
    )
    kinds = [ExprKind.from_into_expr(comparand, str_as_lit=str_as_lit) for comparand in comparands]
