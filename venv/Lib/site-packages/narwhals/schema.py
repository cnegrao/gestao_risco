...
class Schema(BaseSchema):
    ...
    def to_arrow(self) -> pa.Schema:
        ...
        return pa.schema(
            (name, narwhals_to_native_dtype(dtype, self._version)) for name, dtype in self.items()
        )
    ...
    def to_polars(self) -> pl.Schema:
        ...
        schema = (
            (
                name,
                narwhals_to_native_dtype(dtype, self._version, backend_version=pl_version),
            )
            for name, dtype in self.items()
        )
        return pl.Schema(schema) if pl_version >= (1, 0, 0) else cast("pl.Schema", dict(schema))
