...
class DuckDBLazyFrame(CompliantLazyFrame["DuckDBExpr", "duckdb.DuckDBPyRelation"]):
    ...
    @classmethod
    def from_native(cls, data: duckdb.DuckDBPyRelation, /, *, context: _FullContext) -> Self:
        return cls(data, backend_version=context._backend_version, version=context._version)
    ...
    def columns(self) -> list[str]:
        if self._cached_columns is None:
            self._cached_columns = (
                list(self.schema) if self._cached_schema is not None else self.native.columns
            )
    ...
    def _with_version(self, version: Version) -> Self:
        return self.__class__(self.native, version=version, backend_version=self._backend_version)

    def _with_native(self, df: duckdb.DuckDBPyRelation) -> Self:
        return self.__class__(df, backend_version=self._backend_version, version=self._version)
    ...
    def rename(self, mapping: Mapping[str, str]) -> Self:
        df = self.native
        selection = (
            col(name).alias(mapping[name]) if name in mapping else col(name) for name in df.columns
        )
        return self._with_native(self.native.select(*selection))
    ...
    def join_asof(
        ...
        *,
        ...
        for name in rhs.columns:
            if name in lhs.columns and (right_on is None or name not in {right_on, *by_right}):
    ...
    def unique(self, subset: Sequence[str] | None, *, keep: LazyUniqueKeepStrategy) -> Self:
        if subset_ := subset if keep == "any" else (subset or self.columns):
            ...
            if any(x not in self.columns for x in subset_):
                msg = (
                    f"Columns {set(subset_).difference(self.columns)} not found in {self.columns}."
                )
    ...
    def explode(self, columns: Sequence[str]) -> Self:
        ...
        null_rel = rel.filter(~not_null_condition).select(
            *(lit(None).alias(name) if name in columns else name for name in original_columns)
        )
    ...
    def unpivot(
        ...
        query = f"""
            ...
            into
                ...
                value "{value_name}"
            """
        return self._with_native(duckdb.sql(query).select(*[*index_, variable_name, value_name]))
