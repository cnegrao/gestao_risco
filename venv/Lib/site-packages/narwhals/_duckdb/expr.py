...
class DuckDBExpr(LazyExpr["DuckDBLazyFrame", "duckdb.Expression"]):
    ...
    def __narwhals_namespace__(self) -> DuckDBNamespace:  # pragma: no cover
        ...
        return DuckDBNamespace(backend_version=self._backend_version, version=self._version)
    ...
    def _cum_window_func(
        ...
        *,
        ...
        def func(window_inputs: WindowInputs) -> duckdb.Expression:
            order_by_sql = generate_order_by_sql(*window_inputs.order_by, ascending=not reverse)
    ...
    def __floordiv__(self, other: DuckDBExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__floordiv__(other), other=other)
    ...
    def n_unique(self) -> Self:
        def func(_input: duckdb.Expression) -> duckdb.Expression:
            ...
            ) + FunctionExpression("max", when(_input.isnotnull(), lit(0)).otherwise(lit(1)))
    ...
    def std(self, ddof: int) -> Self:
        if ddof == 0:
            return self._with_callable(lambda _input: FunctionExpression("stddev_pop", _input))
        if ddof == 1:
            return self._with_callable(lambda _input: FunctionExpression("stddev_samp", _input))
    ...
    def var(self, ddof: int) -> Self:
        if ddof == 0:
            return self._with_callable(lambda _input: FunctionExpression("var_pop", _input))
        if ddof == 1:
            return self._with_callable(lambda _input: FunctionExpression("var_samp", _input))

        def _var(_input: duckdb.Expression) -> duckdb.Expression:
            n_samples = FunctionExpression("count", _input)
            return FunctionExpression("var_pop", _input) * n_samples / (n_samples - lit(ddof))
    ...
    def over(self, partition_by: Sequence[str], order_by: Sequence[str] | None) -> Self:
        if (window_function := self._window_function) is not None:
            ...
            def func(df: DuckDBLazyFrame) -> list[duckdb.Expression]:
                return [SQLExpression(template.format(expr=expr)) for expr in self._call(df)]
    ...
    def shift(self, n: int) -> Self:
        ...
        def func(window_inputs: WindowInputs) -> duckdb.Expression:
            order_by_sql = generate_order_by_sql(*window_inputs.order_by, ascending=True)
            partition_by_sql = generate_partition_by_sql(*window_inputs.partition_by)
            sql = f"lag({window_inputs.expr}, {n}) over ({partition_by_sql} {order_by_sql})"
            return SQLExpression(sql)  # type: ignore[no-any-return, unused-ignore]
