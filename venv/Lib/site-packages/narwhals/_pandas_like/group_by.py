...
class PandasLikeGroupBy(EagerGroupBy["PandasLikeDataFrame", "PandasLikeExpr"]):
    ...
    def __init__(
        ...
        *,
        ...
        self._compliant_frame, self._keys, self._output_key_names = self._parse_keys(df, keys=keys)
        ...
        if self.compliant._implementation.is_pandas() and self.compliant._backend_version < (
            1,
            1,
        ):  # pragma: no cover
            if (
    ...
    def agg(self, *exprs: PandasLikeExpr) -> PandasLikeDataFrame:  # noqa: PLR0915
        ...
        std_aggs: dict[int, tuple[list[str], list[str]]] = collections.defaultdict(lambda: ([], []))
        ...
        if all_aggs_are_simple:
            ...
            if simple_aggs:
                ...
                if (
                    ...
                    result_simple_aggs.columns = [f"{a}_{b}" for a, b in result_simple_aggs.columns]
                if not (
                ...
                index_map: list[int] = [
                    expected_old_names_indices[item].pop(0) for item in result_simple_aggs.columns
                ]
        ...
        return self.compliant._with_native(
            select_columns_by_name(result_complex, new_names, backend_version, implementation)
        ).rename(dict(zip(self._keys, self._output_key_names)))
