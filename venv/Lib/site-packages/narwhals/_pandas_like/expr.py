...
class PandasLikeExpr(EagerExpr["PandasLikeDataFrame", PandasLikeSeries]):
    ...
    def over(  # noqa: PLR0915
        ...
        if not partition_by:
            ...
            def func(df: PandasLikeDataFrame) -> Sequence[PandasLikeSeries]:
                ...
                df = df.with_row_index(token).sort(*order_by, descending=False, nulls_last=False)
            ...
            def func(df: PandasLikeDataFrame) -> Sequence[PandasLikeSeries]:
                ...
                if function_name.startswith("rolling"):
                    ...
                    if self._call_kwargs["strategy"] == "forward":
                        ...
                        msg = (
                            "`fill_null` with `over` without `strategy` specified is not supported."
                        )
                        raise NotImplementedError(msg)
                ...
                result_frame = df._with_native(res_native).rename(dict(zip(output_names, aliases)))
    ...
    def rank(self, method: RankMethod, *, descending: bool) -> Self:
        return self._reuse_series("rank", call_kwargs={"method": method, "descending": descending})
