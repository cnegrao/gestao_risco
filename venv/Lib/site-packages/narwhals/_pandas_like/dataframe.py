...
class PandasLikeDataFrame(
    ...
    def from_numpy(
        ...
        *,
        ...
        if isinstance(schema, (Mapping, Schema)):
            it: Iterable[DTypeBackend] = (
                get_dtype_backend(native_type, implementation) for native_type in schema.values()
            )
            native = DataFrame(data, columns=schema.keys()).astype(Schema(schema).to_pandas(it))
        else:
    ...
    def _select_slice_name(self, columns: _SliceName) -> Self:
        start = self.native.columns.get_loc(columns.start) if columns.start is not None else None
        stop = self.native.columns.get_loc(columns.stop) + 1 if columns.stop is not None else None
        selector = slice(start, stop, columns.step)
        return self._with_native(self.native.iloc[:, selector], validate_column_names=False)

    def _select_slice_index(self, columns: _SliceIndex | range) -> Self:
        return self._with_native(self.native.iloc[:, columns], validate_column_names=False)

    def _select_multi_index(self, columns: SizedMultiIndexSelector[pd.Series[Any]]) -> Self:
    ...
    def schema(self) -> dict[str, DType]:
        ...
        return {
            col: (
                native_to_narwhals_dtype(native_dtypes[col], self._version, self._implementation)
    ...
    def drop_nulls(self: PandasLikeDataFrame, subset: Sequence[str] | None) -> PandasLikeDataFrame:
        if subset is None:
            return self._with_native(self.native.dropna(axis=0), validate_column_names=False)
    ...
    def filter(
        ...
        return self._with_native(self.native.loc[mask_native], validate_column_names=False)

    def with_columns(self: PandasLikeDataFrame, *exprs: PandasLikeExpr) -> PandasLikeDataFrame:
    ...
    def drop(self, columns: Sequence[str], *, strict: bool) -> Self:
        to_drop = parse_columns_to_drop(compliant_frame=self, columns=columns, strict=strict)
        return self._with_native(self.native.drop(columns=to_drop), validate_column_names=False)
    ...
    def join(
        ...
        *,
        ...
        if how == "cross":
            ...
            ) or (self._implementation is Implementation.PANDAS and self._backend_version < (1, 4)):
        ...
        if how == "semi":
            ...
            return self._with_native(
                self.native.merge(other_native, how="inner", left_on=left_on, right_on=left_on)
            )
    ...
    # --- lazy-only ---
    def lazy(self, *, backend: Implementation | None = None) -> CompliantLazyFrame[Any, Any]:
        from narwhals.utils import parse_version
    ...
    def is_unique(self) -> PandasLikeSeries:
        return PandasLikeSeries.from_native(~self.native.duplicated(keep=False), context=self)
    ...
    def pivot(
        ...
        *,
        ...
        if sort_columns and self._implementation is Implementation.CUDF:
            uniques = {col: sorted(self.native[col].unique().to_arrow().to_pylist()) for col in on}
        elif sort_columns:
            uniques = {col: sorted(self.native[col].unique().tolist()) for col in on}
        elif self._implementation is Implementation.CUDF:
            uniques = {col: self.native[col].unique().to_arrow().to_pylist() for col in on}
        else:
        ...
        if n_on == 1:
            new_columns = [
                separator.join(col).strip() if len(values) > 1 else col[-1] for col in columns
            ]
        else:
            new_columns = [
                (
    ...
    def sample(
        ...
        *,
        ...
        return self._with_native(
            self.native.sample(n=n, frac=fraction, replace=with_replacement, random_state=seed),
            validate_column_names=False,
        )
    ...
    def explode(self, columns: Sequence[str]) -> Self:
        ...
        for col_to_explode in columns:
            ...
            if dtype != dtypes.List:
                msg = (
                    f"`explode` operation not supported for dtype `{dtype}`, " "expected List type"
                )
                raise InvalidOperationError(msg)

        if len(columns) == 1:
            return self._with_native(self.native.explode(columns[0]), validate_column_names=False)
        else:
