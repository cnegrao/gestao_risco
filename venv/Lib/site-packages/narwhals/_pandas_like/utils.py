...
def set_index(
    ...
    *,
    ...
    if implementation is Implementation.CUDF:  # pragma: no cover
        ...
        return obj
    if implementation is Implementation.PANDAS and (backend_version < (1,)):  # pragma: no cover
...
def rename(
    ...
    **kwargs: Any,
    ...
    if implementation is Implementation.PANDAS and (backend_version >= (3,)):  # pragma: no cover
...
def non_object_native_to_narwhals_dtype(native_dtype: Any, version: Version) -> DType:
    ...
    if (match_ := PATTERN_PD_DATETIME.match(dtype)) or (match_ := PATTERN_PA_DATETIME.match(dtype)):
    ...
    if (match_ := PATTERN_PD_DURATION.match(dtype)) or (match_ := PATTERN_PA_DURATION.match(dtype)):
...
def narwhals_to_native_dtype(  # noqa: PLR0915
    ...
    if isinstance_or_issubclass(dtype, dtypes.Datetime):
        ...
        if implementation is Implementation.PANDAS and backend_version < (2,):  # pragma: no cover
            dt_time_unit = "ns"
        else:
