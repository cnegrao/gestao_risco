...
class PandasLikeNamespace(
    ...
    def lit(self, value: NonNestedLiteral, dtype: DType | type[DType] | None) -> PandasLikeExpr:
    ...
    def all_horizontal(self, *exprs: PandasLikeExpr) -> PandasLikeExpr:
        def func(df: PandasLikeDataFrame) -> list[PandasLikeSeries]:
            series = align_series_full_broadcast(*(s for _expr in exprs for s in _expr(df)))
    ...
    def min_horizontal(self, *exprs: PandasLikeExpr) -> PandasLikeExpr:
        def func(df: PandasLikeDataFrame) -> list[PandasLikeSeries]:
            ...
            return [
                PandasLikeSeries(
                    self.concat((s.to_frame() for s in series), how="horizontal")._native_frame.min(
                        axis=1
                    ),
                    implementation=self._implementation,
                    backend_version=self._backend_version,
    ...
    def max_horizontal(self, *exprs: PandasLikeExpr) -> PandasLikeExpr:
        def func(df: PandasLikeDataFrame) -> list[PandasLikeSeries]:
            ...
            return [
                PandasLikeSeries(
                    self.concat((s.to_frame() for s in series), how="horizontal")._native_frame.max(
                        axis=1
                    ),
                    implementation=self._implementation,
                    backend_version=self._backend_version,
        ...
        *exprs: PandasLikeExpr,
        ...
        def func(df: PandasLikeDataFrame) -> list[PandasLikeSeries]:
            ...
            if not ignore_nulls:
                ...
                init_value, *values = [s.zip_with(~nm, "") for s, nm in zip(series, null_mask)]
