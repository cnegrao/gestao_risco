...
class PandasLikeSeries(EagerSeries[Any]):
    ...
    def __narwhals_namespace__(self) -> PandasLikeNamespace:
        ...
        return PandasLikeNamespace(self._implementation, self._backend_version, self._version)
    ...
    def _gather_slice(self, rows: _SliceIndex | range) -> Self:
        return self._with_native(self.native.iloc[slice(rows.start, rows.stop, rows.step)])
    ...
    def dtype(self) -> DType:
        ...
        return (
            ...
            else object_native_to_narwhals_dtype(self.native, self._version, self._implementation)
        )
    ...
    def _scatter_in_place(self, indices: Self, values: Self) -> None:
        ...
        min_pd_version = (1, 2)
        if self._implementation is Implementation.PANDAS and self._backend_version < min_pd_version:
    ...
    def fill_null(
        ...
        if value is not None:
            ...
            res_ser = self._with_native(ser.fillna(value=native_value), preserve_broadcast=True)
        else:
            res_ser = self._with_native(
                ser.ffill(limit=limit) if strategy == "forward" else ser.bfill(limit=limit),
    ...
    def unique(self, *, maintain_order: bool) -> PandasLikeSeries:
        ...
        return self._with_native(self.native.__class__(self.native.unique(), name=self.name))
    ...
    def replace_strict(
        ...
        *,
        ...
        other = namespace.DataFrame({self.name: old, tmp_name: namespace.Series(new, dtype=dtype)})
    ...
    def clip(
        ...
        _, lower = align_and_extract_native(self, lower_bound) if lower_bound else (None, None)
        _, upper = align_and_extract_native(self, upper_bound) if upper_bound else (None, None)
