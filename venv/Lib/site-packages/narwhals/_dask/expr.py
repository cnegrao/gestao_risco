...
class DaskExpr(
    ...
    def from_column_names(
        ...
        *,
        ...
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            try:
                return [df._native_frame[column_name] for column_name in evaluate_column_names(df)]
            except KeyError as e:
                missing_columns = [x for x in evaluate_column_names(df) if x not in df.columns]
    ...
    def from_column_indices(cls: type[Self], *column_indices: int, context: _FullContext) -> Self:
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            return [df._native_frame.iloc[:, column_index] for column_index in column_indices]
    ...
    def _with_callable(
        ...
        **expressifiable_args: Self | Any,
        ...
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            ...
            other_native_series = {
                key: maybe_evaluate_expr(df, value) for key, value in expressifiable_args.items()
            }
    ...
    def is_nan(self) -> Self:
        def func(_input: dx.Series) -> dx.Series:
            dtype = native_to_narwhals_dtype(_input.dtype, self._version, self._implementation)
            if dtype.is_numeric():
        ...
        if interpolation == "linear":
            ...
            def func(_input: dx.Series, quantile: float) -> dx.Series:
                if _input.npartitions > 1:
                    ...
                    raise NotImplementedError(msg)
                return _input.quantile(q=quantile, method="dask").to_series()  # pragma: no cover
    ...
    def is_unique(self) -> Self:
        def func(_input: dx.Series) -> dx.Series:
            _name = _input.name
            return (
                _input.to_frame().groupby(_name, dropna=False).transform("size", meta=(_name, int))
                == 1
            )
    ...
    def null_count(self) -> Self:
        return self._with_callable(lambda _input: _input.isna().sum().to_series(), "null_count")
