...
class DaskNamespace(
    ...
    def all_horizontal(self, *exprs: DaskExpr) -> DaskExpr:
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            series = align_series_full_broadcast(df, *(s for _expr in exprs for s in _expr(df)))
    ...
    def concat(self, items: Iterable[DaskLazyFrame], *, how: ConcatMethod) -> DaskLazyFrame:
        if not items:
    ...
    def mean_horizontal(self, *exprs: DaskExpr) -> DaskExpr:
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            ...
            non_na = align_series_full_broadcast(df, *(1 - s.isna() for s in expr_results))
            return [
                name_preserving_div(
    ...
    def concat_str(
        ...
        *exprs: DaskExpr,
        ...
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            expr_results = [s for _expr in exprs for s in _expr(df)]
            series = (s.astype(str) for s in align_series_full_broadcast(df, *expr_results))
            ...
            if not ignore_nulls:
                ...
                init_value, *values = [s.where(~nm, "") for s, nm in zip(series, null_mask)]

                separators = (
                    nm.map({True: "", False: separator}, meta=str) for nm in null_mask[:-1]
                )
                result = reduce(
                    operator.add,
...
class DaskWhen(CompliantWhen[DaskLazyFrame, "dx.Series", DaskExpr]):
    ...
    def __call__(self, df: DaskLazyFrame) -> Sequence[dx.Series]:
        ...
        if isinstance(self._otherwise_value, DaskExpr):
            ...
            return [
                then_series.where(condition, self._otherwise_value)
