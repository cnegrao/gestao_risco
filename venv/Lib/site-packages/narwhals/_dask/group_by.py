...
class DaskLazyGroupBy(DepthTrackingGroupBy["DaskLazyFrame", "DaskExpr", Aggregation]):
    ...
    def agg(self, *exprs: DaskExpr) -> DaskLazyFrame:
        ...
        for expr in exprs:
            output_names, aliases = evaluate_output_names_and_aliases(expr, self.compliant, exclude)
            if expr._depth == 0:
                # e.g. `agg(nw.len())`
                column = self._keys[0]
            ...
            simple_aggregations.update(
                (alias, (output_name, agg_fn)) for alias, output_name in zip(aliases, output_names)
            )
