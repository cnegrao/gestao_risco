...
def is_modin_index(index: Any) -> TypeIs[mpd.Index]:
    ...
    return (mpd := get_modin()) is not None and isinstance(index, mpd.Index)  # pragma: no cover
...
def is_cudf_index(index: Any) -> TypeIs[cudf.Index]:
    ...
    return (cudf := get_cudf()) is not None and isinstance(index, cudf.Index)  # pragma: no cover
...
def is_duckdb_relation(df: Any) -> TypeIs[duckdb.DuckDBPyRelation]:
    ...
    return (duckdb := get_duckdb()) is not None and isinstance(df, duckdb.DuckDBPyRelation)
...
def is_pyspark_dataframe(df: Any) -> TypeIs[pyspark_sql.DataFrame]:
    ...
    return bool(
        (pyspark_sql := get_pyspark_sql()) is not None and isinstance(df, pyspark_sql.DataFrame)
    )
...
def is_numpy_array_1d_int(arr: Any) -> TypeIs[_1DArrayInt]:
    return (np := get_numpy()) and is_numpy_array_1d(arr) and np.issubdtype(arr.dtype, np.integer)
