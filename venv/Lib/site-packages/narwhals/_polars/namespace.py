...
class PolarsNamespace:
    ...
    @requires.backend_version((1, 0, 0), "Please use `col` for columns selection instead.")
    ...
    def len(self) -> PolarsExpr:
        ...
        return self._expr(pl.len(), version=self._version, backend_version=self._backend_version)
    ...
    def lit(self, value: Any, dtype: DType | type[DType] | None) -> PolarsExpr:
        if dtype is not None:
            return self._expr(
                pl.lit(
                    value,
                    dtype=narwhals_to_native_dtype(dtype, self._version, self._backend_version),
                ),
                version=self._version,
                backend_version=self._backend_version,
            )
        ...
        *exprs: PolarsExpr,
        ...
        if self._backend_version < (0, 20, 6):
            ...
            if not ignore_nulls:
                ...
                separators = [pl.when(~nm).then(sep).otherwise(pl.lit("")) for nm in null_mask[:-1]]
            ...
            return self._expr(result, version=self._version, backend_version=self._backend_version)
...
class PolarsSelectorNamespace:
    ...
    def by_dtype(self, dtypes: Iterable[DType]) -> PolarsExpr:
        native_dtypes = [
            (
                narwhals_to_native_dtype(dtype, self._version, self._backend_version).__class__
