...
class PolarsExpr:
    def __init__(self, expr: pl.Expr, version: Version, backend_version: tuple[int, ...]) -> None:
    ...
    def rolling_var(self, window_size: int, *, min_samples: int, center: bool, ddof: int) -> Self:
        ...
        native = self.native.rolling_var(window_size=window_size, center=center, ddof=ddof, **kwds)
        return self._with_native(native)

    @requires.backend_version((1,))
    def rolling_std(self, window_size: int, *, min_samples: int, center: bool, ddof: int) -> Self:
        kwds = self._renamed_min_periods(min_samples)
        native = self.native.rolling_std(window_size=window_size, center=center, ddof=ddof, **kwds)
        return self._with_native(native)
...
class PolarsExprListNamespace:
    ...
    def len(self) -> PolarsExpr:
        ...
        if self._expr._backend_version < (1, 16):  # pragma: no cover
            native_result = pl.when(~native_expr.is_null()).then(native_result).cast(pl.UInt32())
    ...
    def __getattr__(self, attr: str) -> Callable[[Any], PolarsExpr]:  # pragma: no cover
        def func(*args: Any, **kwargs: Any) -> PolarsExpr:
            pos, kwds = extract_args_kwargs(args, kwargs)
            return self._expr._with_native(getattr(self._expr.native.list, attr)(*pos, **kwds))
...
class PolarsExprStructNamespace:
    ...
    def __getattr__(self, attr: str) -> Callable[[Any], PolarsExpr]:  # pragma: no cover
        def func(*args: Any, **kwargs: Any) -> PolarsExpr:
            pos, kwds = extract_args_kwargs(args, kwargs)
            return self._expr._with_native(getattr(self._expr.native.struct, attr)(*pos, **kwds))
