...
class PolarsDataFrame:
    ...
    def from_numpy(
        ...
        *,
        ...
        pl_schema = Schema(schema).to_polars() if isinstance(schema, (Mapping, Schema)) else schema
    ...
    def _from_native_object(self, obj: pl.Series | pl.DataFrame | T) -> Self | PolarsSeries | T:
    ...
    def __array__(self, dtype: Any | None = None, *, copy: bool | None = None) -> _2DArray:
        ...
        if self._backend_version > (0, 20, 30):
            ...
            if not is_slice_none(columns):
                ...
                if is_index_selector(columns):
                    ...
                    native = native.select(
                        self.columns[slice(*convert_str_slice_to_int_slice(columns, self.columns))]
                    )
                elif is_compliant_series(columns):
    ...
    def to_dict(self, *, as_series: bool) -> dict[str, PolarsSeries] | dict[str, list[Any]]:
        if as_series:
            return {
    ...
    def join(
        ...
        *,
        ...
        how_native = "outer" if (self._backend_version < (0, 20, 29) and how == "full") else how
        try:
            return self._with_native(
                self.native.join(
                    other=other.native,
