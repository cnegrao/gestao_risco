...
class Implementation(Enum):
    ...
    @classmethod
    def from_string(cls: type[Self], backend_name: str) -> Implementation:  # pragma: no cover
        """Instantiate Implementation object from a native namespace module.
    ...
    @classmethod
    def from_backend(cls: type[Self], backend: str | Implementation | ModuleType) -> Implementation:
...
@overload
def isinstance_or_issubclass(obj_or_cls: type, cls_or_tuple: type[_T]) -> TypeIs[type[_T]]: ...
...
@overload
def isinstance_or_issubclass(obj_or_cls: Any, cls_or_tuple: tuple[type, ...]) -> TypeIs[Any]: ...
...
def maybe_align_index(
    ...
    if isinstance(getattr(lhs_any, "_compliant_frame", None), PandasLikeDataFrame) and isinstance(
        getattr(rhs_any, "_compliant_frame", None), PandasLikeDataFrame
    ):
    ...
    if isinstance(getattr(lhs_any, "_compliant_frame", None), PandasLikeDataFrame) and isinstance(
        getattr(rhs_any, "_compliant_series", None), PandasLikeSeries
    ):
        _validate_index(lhs_any._compliant_frame.native.index)
        _validate_index(rhs_any._compliant_series.native.index)
        return lhs_any._with_compliant(
            lhs_any._compliant_frame._with_native(
                lhs_any._compliant_frame.native.loc[rhs_any._compliant_series.native.index]
            )
        )
    if isinstance(getattr(lhs_any, "_compliant_series", None), PandasLikeSeries) and isinstance(
        getattr(rhs_any, "_compliant_frame", None), PandasLikeDataFrame
    ):
        ...
        return lhs_any._with_compliant(
            lhs_any._compliant_series._with_native(
                lhs_any._compliant_series.native.loc[rhs_any._compliant_frame.native.index]
            )
        )
    if isinstance(getattr(lhs_any, "_compliant_series", None), PandasLikeSeries) and isinstance(
        getattr(rhs_any, "_compliant_series", None), PandasLikeSeries
        ...
        return lhs_any._with_compliant(
            lhs_any._compliant_series._with_native(
                lhs_any._compliant_series.native.loc[rhs_any._compliant_series.native.index]
            )
        )
...
def maybe_convert_dtypes(obj: FrameOrSeriesT, *args: bool, **kwargs: bool | str) -> FrameOrSeriesT:
    ...
    if is_pandas_like_dataframe(native_obj):
        return obj_any._with_compliant(
            obj_any._compliant_frame._with_native(native_obj.convert_dtypes(*args, **kwargs))
        )
    if is_pandas_like_series(native_obj):
        return obj_any._with_compliant(
            obj_any._compliant_series._with_native(native_obj.convert_dtypes(*args, **kwargs))
        )
    return obj_any
...
def is_ordered_categorical(series: Series[Any]) -> bool:
    ...
    if isinstance(compliant, InterchangeSeries) and isinstance(series.dtype, dtypes.Categorical):
...
def generate_unique_token(n_bytes: int, columns: Sequence[str]) -> str:  # pragma: no cover
    msg = (
...
def is_index_selector(
    ...
    return (
        is_single_index_selector(obj) or is_sized_multi_index_selector(obj) or is_slice_index(obj)
    )
...
def is_sequence_of(obj: Any, tp: type[_T]) -> TypeIs[Sequence[_T]]:
    ...
    return bool(
        is_sequence_but_not_str(obj) and (first := next(iter(obj), None)) and isinstance(first, tp)
    )
...
def _validate_rolling_arguments(window_size: int, min_samples: int | None) -> tuple[int, int]:
    if window_size < 1:
    ...
    if not isinstance(window_size, int):
        ...
        msg = f"argument 'window_size': '{_type}' object cannot be " "interpreted as an integer"
        raise TypeError(msg)
    ...
    if min_samples is not None:
        ...
        if not isinstance(min_samples, int):
            ...
            msg = f"argument 'min_samples': '{_type}' object cannot be " "interpreted as an integer"
            raise TypeError(msg)
...
def generate_repr(header: str, native_repr: str) -> str:
    ...
    if max_native_width + 2 <= terminal_width:
        ...
        for line in native_lines:
            output += (
                f"|{' ' * (start_extra)}{line}{' ' * (end_extra + max_native_width - len(line))}|\n"
            )
        output += f"└{'─' * length}┘"
        return output
...
def dtype_matches_time_unit_and_time_zone(
    ...
    return (
        ...
        and (dtype.time_zone in time_zones or ("*" in time_zones and dtype.time_zone is not None))
    )
...
def _remap_full_join_keys(
    ...
    """
    right_keys_suffixed = (f"{key}{suffix}" if key in left_on else key for key in right_on)
