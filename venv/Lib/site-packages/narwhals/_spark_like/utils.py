...
def narwhals_to_native_dtype(
    ...
    if isinstance_or_issubclass(dtype, (dtypes.List, dtypes.Array)):
        return native.ArrayType(
            elementType=narwhals_to_native_dtype(dtype.inner, version=version, spark_types=native)
        )
...
def evaluate_exprs(df: SparkLikeLazyFrame, /, *exprs: SparkLikeExpr) -> list[tuple[str, Column]]:
...
def import_window(implementation: Implementation, /) -> type[Any]:
    ...
    return import_module(f"sqlframe.{_BaseSession().execution_dialect_name}.window").Window
