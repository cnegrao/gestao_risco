...
class SparkLikeLazyFrame(CompliantLazyFrame["SparkLikeExpr", "SQLFrameDataFrame"]):
    ...
    def sort(
        ...
        *by: str,
        ...
        if nulls_last:
            sort_funcs = (
                self._F.desc_nulls_last if d else self._F.asc_nulls_last for d in descending
            )
        else:
            sort_funcs = (
                self._F.desc_nulls_first if d else self._F.asc_nulls_first for d in descending
            )
    ...
    def rename(self, mapping: Mapping[str, str]) -> Self:
        rename_mapping = {colname: mapping.get(colname, colname) for colname in self.columns}
        return self._with_native(
            self.native.select([self._F.col(old).alias(new) for old, new in rename_mapping.items()])
        )
    ...
    def join(
        ...
        right_cols_to_rename = (
            [c for c in right_columns if c not in right_on_] if how != "full" else right_columns
        )
        ...
        if how in {"inner", "left", "cross"}:
            col_order.extend(
                rename_mapping[colname] for colname in right_columns if colname not in right_on_
            )
        elif how == "full":
    ...
    def unpivot(
        ...
        values = tuple(set(self.columns).difference(set(ids))) if on is None else tuple(on)
