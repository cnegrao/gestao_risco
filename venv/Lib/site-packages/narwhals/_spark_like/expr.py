...
class SparkLikeExpr(LazyExpr["SparkLikeLazyFrame", "Column"]):
    ...
    def broadcast(self, kind: Literal[ExprKind.AGGREGATION, ExprKind.LITERAL]) -> Self:
        ...
        def func(df: SparkLikeLazyFrame) -> Sequence[Column]:
            return [result.over(self._Window().partitionBy(self._F.lit(1))) for result in self(df)]
    ...
    def _cum_window_func(
        ...
        *,
        ...
        def func(window_inputs: WindowInputs) -> Column:
            if reverse:
                order_by_cols = [self._F.col(x).desc_nulls_last() for x in window_inputs.order_by]
            else:
                order_by_cols = [self._F.col(x).asc_nulls_first() for x in window_inputs.order_by]
            window = (
                self._Window()
        ...
        *,
        ...
        def func(window_inputs: WindowInputs) -> Column:
            window = (
                self._Window()
                .partitionBy(list(window_inputs.partition_by))
                .orderBy([self._F.col(x).asc_nulls_first() for x in window_inputs.order_by])
    ...
    @classmethod
    def from_column_indices(cls: type[Self], *column_indices: int, context: _FullContext) -> Self:
        def func(df: SparkLikeLazyFrame) -> list[Column]:
            columns = df.columns
    ...
    def _with_callable(
        ...
        **expressifiable_args: Self | Any,
        ...
        def func(df: SparkLikeLazyFrame) -> list[Column]:
            ...
            return [
                call(native_series, **other_native_series) for native_series in native_series_list
            ]
    ...
    def __eq__(self, other: SparkLikeExpr) -> Self:  # type: ignore[override]
        return self._with_callable(lambda _input, other: _input.__eq__(other), other=other)

    def __ne__(self, other: SparkLikeExpr) -> Self:  # type: ignore[override]
        return self._with_callable(lambda _input, other: _input.__ne__(other), other=other)

    def __add__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__add__(other), other=other)

    def __sub__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__sub__(other), other=other)

    def __rsub__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: other.__sub__(_input), other=other).alias(
            "literal"
        )

    def __mul__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__mul__(other), other=other)

    def __truediv__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__truediv__(other), other=other)
    ...
    def __pow__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__pow__(other), other=other)

    def __rpow__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: other.__pow__(_input), other=other).alias(
            "literal"
        )

    def __mod__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__mod__(other), other=other)

    def __rmod__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: other.__mod__(_input), other=other).alias(
            "literal"
        )

    def __ge__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__ge__(other), other=other)
    ...
    def __le__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__le__(other), other=other)

    def __lt__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__lt__(other), other=other)

    def __and__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__and__(other), other=other)

    def __or__(self, other: SparkLikeExpr) -> Self:
        return self._with_callable(lambda _input, other: _input.__or__(other), other=other)
    ...
    def cast(self, dtype: DType | type[DType]) -> Self:
        def _cast(_input: Column) -> Column:
            spark_dtype = narwhals_to_native_dtype(dtype, self._version, self._native_dtypes)
    ...
    def median(self) -> Self:
        def _median(_input: Column) -> Column:
            if (
                self._implementation in {Implementation.PYSPARK, Implementation.PYSPARK_CONNECT}
    ...
    def clip(
        ...
        def _clip_both(_input: Column, lower_bound: Column, upper_bound: Column) -> Column:
            return (
        ...
        return self._with_callable(_clip_both, lower_bound=lower_bound, upper_bound=upper_bound)
    ...
    def is_finite(self) -> Self:
        def _is_finite(_input: Column) -> Column:
            ...
            return self._F.when(~self._F.isnull(_input), is_finite_condition).otherwise(None)
    ...
    def over(self, partition_by: Sequence[str], order_by: Sequence[str] | None) -> Self:
        if (window_function := self._window_function) is not None:
            ...
            def func(df: SparkLikeLazyFrame) -> list[Column]:
                return [
                    window_function(WindowInputs(expr, partition_by or [self._F.lit(1)], order_by))
            ...
            def func(df: SparkLikeLazyFrame) -> list[Column]:
                return [
                    expr.over(self._Window.partitionBy(*partition_by)) for expr in self._call(df)
                ]
    ...
    def is_nan(self) -> Self:
        def _is_nan(_input: Column) -> Column:
            return self._F.when(self._F.isnull(_input), None).otherwise(self._F.isnan(_input))
    ...
    def shift(self, n: int) -> Self:
        def func(window_inputs: WindowInputs) -> Column:
            order_by_cols = [self._F.col(x).asc_nulls_first() for x in window_inputs.order_by]
            window = (
                self._Window().partitionBy(list(window_inputs.partition_by)).orderBy(order_by_cols)
            )
    ...
    def cum_sum(self, *, reverse: bool) -> Self:
        return self._with_window_function(self._cum_window_func(reverse=reverse, func_name="sum"))

    def cum_max(self, *, reverse: bool) -> Self:
        return self._with_window_function(self._cum_window_func(reverse=reverse, func_name="max"))

    def cum_min(self, *, reverse: bool) -> Self:
        return self._with_window_function(self._cum_window_func(reverse=reverse, func_name="min"))

    def cum_count(self, *, reverse: bool) -> Self:
        return self._with_window_function(self._cum_window_func(reverse=reverse, func_name="count"))
    ...
    def fill_null(
        ...
        if strategy is not None:
            ...
            def _fill_with_strategy(window_inputs: WindowInputs) -> Column:
                fill_func = self._F.last_value if strategy == "forward" else self._F.first_value

                if strategy == "forward":
                    start = -limit if limit is not None else self._Window().unboundedPreceding
                    end = self._Window().currentRow
                else:
    ...
    def rolling_var(self, window_size: int, *, min_samples: int, center: bool, ddof: int) -> Self:
        return self._with_window_function(
            self._rolling_window_func(
                func_name="var",
                center=center,
    ...
    def rolling_std(self, window_size: int, *, min_samples: int, center: bool, ddof: int) -> Self:
        return self._with_window_function(
            self._rolling_window_func(
                func_name="std",
                center=center,
