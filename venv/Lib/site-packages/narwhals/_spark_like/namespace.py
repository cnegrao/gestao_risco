...
class SparkLikeNamespace(LazyNamespace[SparkLikeLazyFrame, SparkLikeExpr, "SQLFrameDataFrame"]):
    def __init__(
        self,
        *,
    ...
    def lit(self, value: NonNestedLiteral, dtype: DType | type[DType] | None) -> SparkLikeExpr:
    ...
    def sum_horizontal(self, *exprs: SparkLikeExpr) -> SparkLikeExpr:
        def func(df: SparkLikeLazyFrame) -> list[Column]:
            cols = (df._F.coalesce(col, df._F.lit(0)) for _expr in exprs for col in _expr(df))
    ...
    def mean_horizontal(self, *exprs: SparkLikeExpr) -> SparkLikeExpr:
        def func(df: SparkLikeLazyFrame) -> list[Column]:
            ...
            return [
                ...
                / reduce(
                    operator.add,
                    (col.isNotNull().cast(df._native_dtypes.IntegerType()) for col in cols),
                )
            ]
        ...
        def func(df: SparkLikeLazyFrame) -> list[Column]:
            ...
            if not ignore_nulls:
                ...
                result = reduce(
                    ...
                    (df._F.format_string("%s%s", s, v) for s, v in zip(separators, values)),
                    init_value,
                )
...
class SparkLikeThen(CompliantThen[SparkLikeLazyFrame, "Column", SparkLikeExpr], SparkLikeExpr): ...
