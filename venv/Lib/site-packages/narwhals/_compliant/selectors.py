...
class CompliantSelectorNamespace(Protocol[FrameT, SeriesOrExprT]):
    ...
    def _iter_columns_dtypes(self, df: FrameT, /) -> Iterator[tuple[SeriesOrExprT, DType]]: ...
    ...
    def _is_dtype(
        ...
        def series(df: FrameT) -> Sequence[SeriesOrExprT]:
            return [ser for ser, tp in self._iter_columns_dtypes(df) if isinstance(tp, dtype)]
...
class CompliantSelector(CompliantExpr[FrameT, SeriesOrExprT], Protocol38[FrameT, SeriesOrExprT]):
    ...
    def __sub__(
        ...
        if self._is_selector(other):
            ...
            def series(df: FrameT) -> Sequence[SeriesOrExprT]:
                ...
                return [x for x, name in zip(self(df), lhs_names) if name not in rhs_names]
