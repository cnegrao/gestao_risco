...
def _from_native_impl(  # noqa: PLR0915
    ...
    *,
    ...
    if eager_only and eager_or_interchange_only:
        msg = (
            "Invalid parameter combination: `eager_only=True` and `eager_or_interchange_only=True`"
        )
        raise ValueError(msg)
    ...
    # Polars
        ...
        if series_only and not is_polars_series(native_object):
            if not pass_through:
                ...
                raise TypeError(msg)
            return native_object
        if (eager_only or eager_or_interchange_only) and is_polars_lazyframe(native_object):
            if not pass_through:
    ...
    # PyArrow
        ...
        if not allow_series and not is_pyarrow_table(native_object):
            if not pass_through:
                ...
                raise TypeError(msg)
            return native_object
        pa_compliant = version.namespace.from_native_object(native_object).compliant.from_native(
            native_object
        )
    ...
    # Ibis
        ...
        if version is Version.V1:
            return DataFrame(
                IbisLazyFrame(native_object, backend_version=backend_version, version=version),
                level="interchange",
            )
        return LazyFrame(
            IbisLazyFrame(native_object, backend_version=backend_version, version=version),
...
def to_py_scalar(scalar_like: Any) -> Any:
    ...
    np = get_numpy()
    if np and isinstance(scalar_like, np.datetime64) and scalar_like.dtype == "datetime64[ns]":
    ...
    cupy = get_cupy()
    if cupy and isinstance(scalar_like, cupy.ndarray) and scalar_like.size == 1:  # pragma: no cover
        return scalar_like.item()
