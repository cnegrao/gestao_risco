...
class DescriptorPool(object):
    ...
    def __init__(self, descriptor_db=None, use_deprecated_legacy_json_field_conflicts=False):
    ...
    def _CreateDefaultFeatures(self, edition):
        ...
        if found is None:
            raise TypeError(
                "No valid default found for edition %s!" % descriptor_pb2.Edition.Name(edition)
            )
        ...
        if file_proto.name not in self._file_descriptors:
            ...
            for message_type in file_proto.message_type:
                message_desc = self._ConvertMessageDescriptor(
                    message_type, file_proto.package, file_descriptor, scope, file_proto.syntax
                )
                file_descriptor.message_types_by_name[message_desc.name] = message_desc

            for enum_type in file_proto.enum_type:
                file_descriptor.enum_types_by_name[enum_type.name] = self._ConvertEnumDescriptor(
                    enum_type, file_proto.package, file_descriptor, None, scope, True
                )

            for index, extension_proto in enumerate(file_proto.extension):
                extension_desc = self._MakeFieldDescriptor(
                    extension_proto, file_proto.package, index, file_descriptor, is_extension=True
                )
            ...
            for index, service_proto in enumerate(file_proto.service):
                file_descriptor.services_by_name[service_proto.name] = self._MakeServiceDescriptor(
    ...
    def _ConvertMessageDescriptor(
        ...
        extensions = [
            self._MakeFieldDescriptor(extension, desc_name, index, file_desc, is_extension=True)
    ...
    def _MakeServiceDescriptor(self, service_proto, service_index, scope, package, file_desc):
        ...
        methods = [
            self._MakeMethodDescriptor(method_proto, service_name, package, scope, index)
