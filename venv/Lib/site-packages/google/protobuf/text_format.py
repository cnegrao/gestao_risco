...
class _Printer(object):
    ...
    def PrintMessage(self, message):
        ...
        if self.message_formatter and self._TryCustomFormatMessage(message):
            return
        if message.DESCRIPTOR.full_name == _ANY_FULL_TYPE_NAME and self._TryPrintAsAnyMessage(
            message
        ):
            return
    ...
    def _MergeField(self, tokenizer, message):
        ...
        if message_descriptor.full_name == _ANY_FULL_TYPE_NAME and tokenizer.TryConsume("["):
            ...
            self._DetectSilentMarker(
                tokenizer, message_descriptor.full_name, type_url_prefix + "/" + packed_type_name
            )
        ...
        if tokenizer.TryConsume("["):
            ...
            if not message_descriptor.is_extendable:
                raise tokenizer.ParseErrorPreviousToken(
                    'Message type "%s" does not have extensions.' % message_descriptor.full_name
                )
            # pylint: disable=protected-access
