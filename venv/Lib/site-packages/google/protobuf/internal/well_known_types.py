...
def _CheckTimestampValid(seconds, nanos):
    ...
    if nanos < 0 or nanos >= _NANOS_PER_SECOND:
        raise ValueError(
            "Timestamp is not valid: Nanos {} must be in a range " "[0, 999999].".format(nanos)
        )
...
class Duration(object):
    ...
    def FromMicroseconds(self, micros):
        ...
        self._NormalizeDuration(
            micros // _MICROS_PER_SECOND, (micros % _MICROS_PER_SECOND) * _NANOS_PER_MICROSECOND
        )

    def FromMilliseconds(self, millis):
        """Converts milliseconds to Duration."""
        self._NormalizeDuration(
            millis // _MILLIS_PER_SECOND, (millis % _MILLIS_PER_SECOND) * _NANOS_PER_MILLISECOND
        )
    ...
    def ToTimedelta(self) -> datetime.timedelta:
        ...
        return datetime.timedelta(
            seconds=self.seconds, microseconds=_RoundTowardZero(self.nanos, _NANOS_PER_MICROSECOND)
        )
