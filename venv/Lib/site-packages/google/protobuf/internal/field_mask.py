...
class FieldMask(object):
    ...
    def MergeMessage(
        self, source, destination, replace_message_field=False, replace_repeated_field=False
    ):
...
def _CamelCaseToSnakeCase(path_name):
    ...
    for c in path_name:
        if c == "_":
            raise ValueError(
                "Fail to parse FieldMask: Path name " '{0} must not contain "_"s.'.format(path_name)
            )
        if c.isupper():
            result += "_"
            result += c.lower()
