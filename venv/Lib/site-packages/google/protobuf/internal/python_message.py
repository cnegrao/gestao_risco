...
def _IsMapField(field):
    return field.type == _FieldDescriptor.TYPE_MESSAGE and field.message_type._is_map_entry
    ...
    def AddDecoder(is_packed):
        ...
        if is_map_entry:
            ...
            field_decoder = decoder.MapDecoder(
                field_descriptor, _GetInitializeDefaultForMap(field_descriptor), is_message_map
            )
    ...
    if _IsMessageMapField(field):
        ...
        def MakePrimitiveMapDefault(message):
            return containers.ScalarMap(
                message._listener_for_children, key_checker, value_checker, field.message_type
            )
...
def _DefaultValueConstructorForField(field):
    ...
    if field.label == _FieldDescriptor.LABEL_REPEATED:
        if field.has_default_value and field.default_value != []:
            raise ValueError(
                "Repeated field default value not empty list: %s" % (field.default_value)
            )
            ...
            if field.label == _FieldDescriptor.LABEL_REPEATED:
                ...
                if field.cpp_type == _FieldDescriptor.CPPTYPE_MESSAGE:  # Composite
                    ...
                    if field.cpp_type == _FieldDescriptor.CPPTYPE_ENUM:
                        field_value = [
                            _GetIntegerEnumValue(field.enum_type, val) for val in field_value
                        ]
...
def _GetFieldByName(message_descriptor, field_name):
    ...
    try:
        ...
        raise ValueError(
            'Protocol message %s has no "%s" field.' % (message_descriptor.name, field_name)
        )
...
def _AddEqualsMethod(message_descriptor, cls):
    ...
    def __eq__(self, other):
        if self.DESCRIPTOR.full_name == _ListValueFullTypeName and isinstance(other, list):
    ...
    def MergeFrom(self, msg):
        if not isinstance(msg, cls):
            raise TypeError(
                ...
                "expected %s got %s."
                % (_FullyQualifiedClassName(cls), _FullyQualifiedClassName(msg.__class__))
            )
