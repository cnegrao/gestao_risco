...
def Parse(
    ...
    try:
        ...
        raise ParseError("Failed to parse JSON: {0}: {1}.".format(type(e).__name__, str(e))) from e
...
class _Parser(object):
            ...
            try:
                ...
                if name in names:
                    raise ParseError(
                        ...
                        '"{1}" fields at "{2}".'.format(message.DESCRIPTOR.full_name, name, path)
                    )
                names.append(name)
                value = js[name]
                ...
                if _IsMapEntry(field):
                    ...
                    if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_MESSAGE:
                        ...
                        for index, item in enumerate(value):
                            ...
                            if (
                                item is None
                                and sub_message.DESCRIPTOR.full_name != "google.protobuf.Value"
                            ):
                                raise ParseError(
                                    "null is not allowed to be used as an element"
                                    " in a repeated field at {0}.{1}[{2}]".format(path, name, index)
                                )
                            self.ConvertMessage(
    ...
    def _ConvertAnyMessage(self, value, message, path):
        ...
        try:
            ...
            raise ParseError("@type is missing when parsing any message at {0}".format(path)) from e
    ...
    def _ConvertValueMessage(self, value, message, path):
        ...
        if isinstance(value, dict):
            ...
            raise ParseError(
                "Value {0} has unexpected type {1} at {2}".format(value, type(value), path)
            )
        ...
        for key in value:
            ...
            if value_field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_MESSAGE:
                ...
                self._ConvertAndSetScalarToMapKey(
                    message, field, key_value, value[key], path="{0}[{1}]".format(path, key_value)
                )
    ...
    def _ConvertAndSetScalar(self, message, field, js_value, path):
        ...
        try:
            setattr(message, field.name, _ConvertScalarFieldValue(js_value, field, path))
        except EnumStringValueParseError:
            if not self.ignore_unknown_fields:
                raise
...
def _ConvertInteger(value):
    ...
    try:
        ...
        if not f.is_integer():
            raise ParseError('Couldn\'t parse non-integer string: "{0}"'.format(value)) from e
        return int(f)
...
def _ConvertFloat(value, field):
    ...
    if isinstance(value, float):
        ...
        if math.isinf(value):
            if value > 0:
                raise ParseError(
                    "Couldn't parse Infinity or value too large, " 'use quoted "Infinity" instead'
                )
            else:
                raise ParseError(
                    "Couldn't parse -Infinity or value too small, " 'use quoted "-Infinity" instead'
                )
        if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_FLOAT:
            # pylint: disable=protected-access
