...
def main():
    ...
    with st.form(key="search_form"):
        ...
        if st.form_submit_button("🔍 Buscar Riscos"):
            ...
            if term:
                ...
                st.session_state.df_riscos = run_select(sql, (like_q, like_q, like_q))
            else:
    ...
    df_display = df[["id_risco", "nome_risco", "descricao", "criticidade", "nome_processo"]]
    ...
    if (
        ...
        or (isinstance(raw, pd.DataFrame) and raw.empty)
    ):
        selected = []
    else:
        selected = raw.to_dict("records") if isinstance(raw, pd.DataFrame) else raw
    ...
    for row in selected:
        ...
        if sankey_df.empty:
            st.info(f"Nenhuma causa/consequência para o risco {row['id_risco']}")
            continue
        ...
        ys = evenly_spaced_positions(nc) + [0.5] + evenly_spaced_positions(len(effects))
        ...
        fig = go.Figure(
            go.Sankey(
                ...
                node=dict(
                    ...
                    color=["#005f88" if i != nc else "#FF6600" for i in range(len(labels))],
                    x=xs,
                    y=ys,
                ),
                link=dict(
