...
def riscos_controle_avaliacao():
    ...
    riscos_df = run_select("SELECT id_risco, nome_risco FROM tb_riscos ORDER BY id_risco;")
    if riscos_df.empty:
        st.error("Nenhum risco cadastrado.")
        return
    riscos_df["label"] = riscos_df["id_risco"].astype(str) + " – " + riscos_df["nome_risco"]
    escolha = st.selectbox("Selecionar Risco:", riscos_df["label"], key="rca_risco")
    ...
    with col_form:
        ...
        with st.expander("Formulário de Controle", expanded=True):
            with st.form(key="rca_form"):
                ...
                situ_idx = situ_opts.index(selected["situacao_controle"]) if selected else 0
                situ = st.selectbox("Situação:", situ_opts, index=situ_idx)

                exec_opts = exec_df["descricao"].tolist()
                exec_idx = exec_opts.index(selected["execucao_controle"]) if selected else 0
    ...
    with col_grid:
        ...
        if st.button("🗑️ Remover Selecionado", key="rm_ctrl"):
            if not sel_list2:
                ...
                run_query(
                    "DELETE FROM tb_risco_controle WHERE id_controle=%s;",
