...
def riscos_plano_tratamento():
    ...
    with col_form:
        ...
        with st.form(key="pt_form"):
            ...
            if selected:
                default_tipo = int(
                    selected.get("id_tipo_tratamento",
                                 tipo_df.at[0, "id_tipo_tratamento"])
                )
            ...
            descricao = st.text_area(
                "Descrição da Ação",
                value=selected.get("descricao_acao", "") if selected else "",
            )
            hoje = datetime.date.today()
            data_inicio = st.date_input(
                "Data de Início",
                value=(hoje if not selected else pd.to_datetime(
                    selected["data_inicio"]).date()),
            )
            prazo = st.date_input(
                "Prazo Limite",
                value=(
                    hoje if not selected else pd.to_datetime(
            ...
            termino = st.date_input(
                ...
                value=(
                    None
                    if not selected or pd.isna(selected.get("data_real_termino"))
                    else pd.to_datetime(selected["data_real_termino"]).date()
                ),
            )
            comentarios = st.text_area(
                "Comentários", value=selected.get("comentarios", "") if selected else ""
            )
        ...
        if plans_df.empty:
            ...
            if st.button("🗑️ Remover Ação", key="pt_rm"):
                ...
                if not rows:
                    ...
                    run_query(
                        "DELETE FROM tb_plano_tratamento WHERE id_plano=%s;",
                        (int(rows[0]["id_plano"]),),
                    )
